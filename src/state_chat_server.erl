...
listen_user_socket(Frame_Factory, Socket, Name) ->
    case gen_tcp:recv(Socket, 0) of
        {ok, Bytes} ->
            Splitted_Strings = string:tokens(binary_to_list(Bytes), "\r\n"),
            % io:format("Here are the tokens ~p", Splitted_Strings),
            common:map(fun(String) ->
                        Frame_Factory ! String
                       end, Splitted_Strings),
            listen_user_socket(Frame_Factory, Socket, Name);
        {error, closed} ->
